#Aqui trate de arreglar lo de que se sobreponen pero no me gusto
...
def calculate_sides(triangle_points):
    """Calcula los lados a, b, c de un triángulo dados sus 3 puntos A,B,C."""
...
def calculate_triangle_angles(triangle_points):
    """Calcula los 3 ángulos de un triángulo dados sus 3 puntos A,B,C."""
...
def other_vertex(edge, vertex):
    """Dado un edge (tuple de 2 vértices) y un vértice, devuelve el vértice opuesto en esa arista."""
...
file_name = 'simple-polygon-exterior_40_7685a35e.instance.json'
...
triangle_edges_cache = {}  # Cache para no reconstruir aristas
...
# Crear los polígonos de triángulos una sola vez (cache)
...
for triangle in triangles:
    ...
    # Usar polígono cacheado en vez de crearlo de nuevo
    tri_polygon = tri_polygons[sorted_triangle]
    if prepared_boundary.contains(tri_polygon):
        # Calcular lados y ángulos con funciones optimizadas
        ...
        # Usar lados precalculados para incentro y radio
        ...
        # Mapear vértices a sus ángulos α
...
if ENABLE_PLOT:
    for tri_data in triangle_info.values():
        ...
        for angle_key, angle_data in tri_data['angles'].items():
            ...
            # Mostrar α en blanco (descomenta si lo quieres ver)
            # plt.text(
...
for edge, tris in edge_to_triangles.items():
    ...
    for vertex in edge:
        ...
        if ENABLE_PLOT:
            plt.plot([closest_tg[0], proj_point[0]], [closest_tg[1], proj_point[1]], color='orange', linewidth=3, zorder=2)
        ...
        red_targets[tuple(sorted(correct_edge))].append({
            ...
            'parent': None,            # No tiene padre, es punto inicial
            'generation': 0,           # Primera generación
            'from_tg': closest_tg,     # ← Guardamos el punto verde original
            'id': next_point_id
        })
        next_point_id += 1
...
def propagate_red_targets(generation):
    ...
    for tri_key, tri_data in triangle_info.items():
        for edge in tri_data['edges']:
            red_pts = [pt for pt in red_targets.get(edge, []) if pt['generation'] == generation - 1]
            ...
            for pt_data in red_pts:
                ...
                # Verificación: evitar que el nuevo punto caiga casi en el mismo lugar que el punto verde original
                if 'from_tg' in pt_data:
                    if np.linalg.norm(new_pt - pt_data['from_tg']) < 1e-5:
                        print(f"Se descartó un punto en generación {generation} por estar demasiado cerca del punto verde original.")
                        continue  # Demasiado cerca del punto verde original, se descarta
                ...
                new_targets.append((tuple(sorted(new_edge)), {
                    ...
                    'parent': pt_data['id'],     # Registro quién lo generó
...
max_iterations = 3  # o el número que prefieras
for generation in range(1, max_iterations + 1):
    ...
    if num_after == num_before:
        break  # No se generaron puntos nuevos, termina la propagación
